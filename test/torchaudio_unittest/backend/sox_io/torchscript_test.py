...
def py_info_func(filepath: str) -> torchffmpeg.backend.sox_io_backend.AudioMetaData:
    return torchffmpeg.backend.sox_io_backend.info(filepath)


def py_load_func(filepath: str, normalize: bool, channels_first: bool):
    return torchffmpeg.backend.sox_io_backend.load(filepath, normalize=normalize, channels_first=channels_first)
