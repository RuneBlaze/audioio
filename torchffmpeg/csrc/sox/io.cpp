...
c10::optional<std::tuple<torch::Tensor, int64_t>> load_audio_file(
  ...
  return torchffmpeg::sox_effects::apply_effects_file(
...
TORCH_LIBRARY_FRAGMENT(torchffmpeg, m) {
  m.def("torchffmpeg::sox_io_get_info", &torchffmpeg::sox_io::get_info_file);
  m.def(
      "torchffmpeg::sox_io_load_audio_file",
      &torchffmpeg::sox_io::load_audio_file);
  m.def(
      "torchffmpeg::sox_io_save_audio_file",
      &torchffmpeg::sox_io::save_audio_file);
}
