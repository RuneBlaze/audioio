#include <torchffmpeg/csrc/sox/io.h>
#include <torchffmpeg/csrc/sox/pybind/effects.h>
#include <torchffmpeg/csrc/sox/pybind/effects_chain.h>
#include <torchffmpeg/csrc/sox/pybind/io.h>
#include <torchffmpeg/csrc/sox/pybind/utils.h>
...
auto get_info_fileobj(py::object fileobj, c10::optional<std::string> format)
  ...
  const auto capacity = [&]() {
    // NOTE:
    // Use the abstraction provided by `libtorchffmpeg` to access the global
    // config defined by libsox. Directly using `sox_get_globals` function will
    // end up retrieving the static variable defined in `_torchffmpeg`, which is
    // not correct.
...
auto load_audio_fileobj(
  ...
  return torchffmpeg::sox_effects::apply_effects_fileobj(
      std::move(fileobj),
      effects,
      normalize,
      channels_first,
...
void save_audio_fileobj(
  ...
  torchffmpeg::sox_effects_chain::SoxEffectsChainPyBind chain(
