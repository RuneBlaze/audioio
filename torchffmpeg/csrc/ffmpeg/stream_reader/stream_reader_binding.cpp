...
#include <torchffmpeg/csrc/ffmpeg/stream_reader/stream_reader_wrapper.h>
#include <stdexcept>
...
TORCH_LIBRARY_FRAGMENT(torchffmpeg, m) {
  m.def("torchffmpeg::ffmpeg_init", []() { avdevice_register_all(); });
  m.def("torchffmpeg::ffmpeg_get_log_level", []() -> int64_t {
    return static_cast<int64_t>(av_log_get_level());
  });
  m.def("torchffmpeg::ffmpeg_set_log_level", [](int64_t level) {
