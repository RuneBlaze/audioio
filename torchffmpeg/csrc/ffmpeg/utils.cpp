...
TORCH_LIBRARY_FRAGMENT(torchffmpeg, m) {
  m.def("torchffmpeg::ffmpeg_get_versions", &get_versions);
  m.def("torchffmpeg::ffmpeg_get_muxers", []() { return get_muxers(false); });
  m.def(
      "torchffmpeg::ffmpeg_get_demuxers", []() { return get_demuxers(false); });
  m.def("torchffmpeg::ffmpeg_get_input_devices", []() {
    return get_demuxers(true);
  });
  m.def("torchffmpeg::ffmpeg_get_build_config", []() {
    return get_build_config();
  });
  m.def("torchffmpeg::ffmpeg_get_output_devices", []() {
    return get_muxers(true);
  });
  m.def("torchffmpeg::ffmpeg_get_audio_decoders", []() {
    return get_codecs(AVMEDIA_TYPE_AUDIO, false);
  });
  m.def("torchffmpeg::ffmpeg_get_audio_encoders", []() {
    return get_codecs(AVMEDIA_TYPE_AUDIO, true);
  });
  m.def("torchffmpeg::ffmpeg_get_video_decoders", []() {
    return get_codecs(AVMEDIA_TYPE_VIDEO, false);
  });
  m.def("torchffmpeg::ffmpeg_get_video_encoders", []() {
    return get_codecs(AVMEDIA_TYPE_VIDEO, true);
  });
  m.def("torchffmpeg::ffmpeg_get_input_protocols", []() {
    return get_protocols(false);
  });
  m.def("torchffmpeg::ffmpeg_get_output_protocols", []() {
