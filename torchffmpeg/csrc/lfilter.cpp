...
#ifdef USE_CUDA
#include <torchffmpeg/csrc/iir_cuda.h>
#endif
...
TORCH_LIBRARY_FRAGMENT(torchffmpeg, m) {
  m.def("torchffmpeg::_lfilter_core_loop", &cpu_lfilter_core_loop);
}

TORCH_LIBRARY(torchffmpeg, m) {
  m.def(
      "torchffmpeg::_lfilter(Tensor waveform, Tensor a_coeffs, Tensor b_coeffs) -> Tensor");
}

TORCH_LIBRARY_IMPL(torchffmpeg, CompositeImplicitAutograd, m) {
  m.impl("torchffmpeg::_lfilter", lfilter_core);
}
