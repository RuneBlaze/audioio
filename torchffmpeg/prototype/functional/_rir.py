...
def simulate_rir_ism(
    ...
    rir = torch.ops.torchffmpeg._simulate_rir(irs, delay_i.type(torch.int32), rir_length)
    ...
    if absorption.shape[0] > 1:
        ...
        filters = torch.ops.torchffmpeg._make_rir_filter(center, sample_rate, n_fft=512)
        rir = torchffmpeg.functional.fftconvolve(rir, filters.unsqueeze(1).repeat(1, rir.shape[1], 1), mode="same")
