...
def _is_backend_dispatcher_enabled() -> bool:
    return os.getenv("TORCHFFMPEG_USE_BACKEND_DISPATCHER") == "1"
...
def list_audio_backends() -> List[str]:
    ...
    if torchffmpeg._extension._SOX_INITIALIZED:
...
def set_audio_backend(backend: Optional[str]):
    ...
    for func in ["save", "load", "info"]:
        setattr(torchffmpeg, func, getattr(module, func))
...
def get_audio_backend() -> Optional[str]:
    ...
    if torchffmpeg.load == no_backend.load:
        return None
    if torchffmpeg.load == sox_io_backend.load:
        return "sox_io"
    if torchffmpeg.load == soundfile_backend.load:
        return "soundfile"
