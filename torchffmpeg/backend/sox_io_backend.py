...
import torch
import torchffmpeg
from torchffmpeg.utils.sox_utils import get_buffer_size
...
if torchffmpeg._extension._FFMPEG_INITIALIZED:
    import torchffmpeg.io._compat as _compat
...
def info(
    ...
    if not torch.jit.is_scripting():
        if hasattr(filepath, "read"):
            ...
            sinfo = torchffmpeg.lib._torchffmpeg_sox.get_info_fileobj(filepath, format)
            if sinfo is not None:
                return AudioMetaData(*sinfo)
            return _fallback_info_fileobj(filepath, format, buffer_size)
        filepath = os.fspath(filepath)
    sinfo = torch.ops.torchffmpeg.sox_io_get_info(filepath, format)
    if sinfo is not None:
        return AudioMetaData(*sinfo)
...
def load(
    ...
    Note:
        ...
        * AMR-NB
        ...
        handle natively, your installation of ``torchffmpeg`` has to be linked to ``libsox``
    ...
    if not torch.jit.is_scripting():
        if hasattr(filepath, "read"):
            ...
            warnings.warn(_deprecation_message)
            ret = torchffmpeg.lib._torchffmpeg_sox.load_audio_fileobj(
        ...
        filepath = os.fspath(filepath)
    ret = torch.ops.torchffmpeg.sox_io_load_audio_file(
        filepath, frame_offset, num_frames, normalize, channels_first, format
    )
    if ret is not None:
        return ret
...
def save(
    ...
    Note:
        ...
        ``"flac"``, ``"ogg"`` and ``"vorbis"``), your installation of ``torchffmpeg`` has
    ...
    if not torch.jit.is_scripting():
        if hasattr(filepath, "write"):
            warnings.warn(_deprecation_message)
            torchffmpeg.lib._torchffmpeg_sox.save_audio_fileobj(
                filepath,
                src,
                sample_rate,
                channels_first,
            ...
            return
        filepath = os.fspath(filepath)
    torch.ops.torchffmpeg.sox_io_save_audio_file(
        filepath,
        src,
        sample_rate,
        channels_first,
