...
from torchffmpeg._internal.module_utils import fail_with_message, is_module_available, no_op
...
_IS_TORCHFFMPEG_EXT_AVAILABLE = is_module_available("torchffmpeg.lib._torchffmpeg")
# Kaldi and RIR features are implemented in _torchffmpeg extension, but they can be individually
# turned on/off at build time. Available means that _torchffmpeg is loaded properly, and
# Kaldi or RIR features are found there.
_IS_RIR_AVAILABLE = False
_IS_KALDI_AVAILABLE = False
if _IS_TORCHFFMPEG_EXT_AVAILABLE:
    _load_lib("libtorchffmpeg")

    import torchffmpeg.lib._torchffmpeg  # noqa

    _check_cuda_version()
    _IS_RIR_AVAILABLE = torchffmpeg.lib._torchffmpeg.is_rir_available()
    _IS_KALDI_AVAILABLE = torchffmpeg.lib._torchffmpeg.is_kaldi_available()


# Similar to libtorchffmpeg, sox-related features should be importable when present.
#
...
if is_module_available("torchffmpeg.lib._torchffmpeg_sox"):
    _init_sox()
...
if is_module_available("torchffmpeg.lib._torchffmpeg_ffmpeg"):
    try:
        _init_ffmpeg()
        ...
        # Since the rest of the torchffmpeg works without it, we do not report the
        # error here.
