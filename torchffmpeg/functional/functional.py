...
def spectrogram(
    ...
    if return_complex is not None:
        warnings.warn(
            ...
            "`torchffmpeg.functional.spectrogram(power=None)` always returns a tensor with "
...
def melscale_fbanks(
    ...
    Note:
        ...
        .. image:: https://download.pytorch.org/torchffmpeg/doc-assets/mel_fbanks.png
...
def linear_fbanks(
    ...
    Note:
        ...
        .. image:: https://download.pytorch.org/torchffmpeg/doc-assets/lin_fbanks.png
           :alt: Visualization of generated filter bank
...
@torchffmpeg._extension.fail_if_no_sox
def apply_codec(
    waveform: Tensor,
    sample_rate: int,
    format: str,
    ...
    Args:
        ...
        compression (float or None, optional): Used for formats other than WAV.
            For more details see :py:func:`torchffmpeg.backend.sox_io_backend.save`.
    ...
    """
    bytes = io.BytesIO()
    torchffmpeg.backend.sox_io_backend.save(
        bytes, waveform, sample_rate, channels_first, compression, format, encoding, bits_per_sample
    )
    bytes.seek(0)
    augmented, sr = torchffmpeg.backend.sox_io_backend.load(bytes, channels_first=channels_first, format=format)
    if sr != sample_rate:
        augmented = resample(augmented, sr, sample_rate)
    return augmented


@torchffmpeg._extension.fail_if_no_kaldi
def compute_kaldi_pitch(
    ...
    waveform = waveform.reshape(-1, shape[-1])
    result = torch.ops.torchffmpeg.kaldi_ComputeKaldiPitch(
        waveform,
        sample_rate,
        frame_length,
        frame_shift,
...
def rnnt_loss(
    ...
    costs, _ = torch.ops.torchffmpeg.rnnt_loss(
        logits=logits,
        targets=targets,
...
def deemphasis(waveform, coeff: float = 0.97) -> torch.Tensor:
    ...
    return torchffmpeg.functional.lfilter(waveform, a_coeffs=a_coeffs, b_coeffs=b_coeffs)
