...
import torch
import torchffmpeg
from torchffmpeg._internal.module_utils import deprecated
from torchffmpeg.utils.sox_utils import list_effects
...
def effect_names() -> List[str]:
    ...
    Example
        >>> torchffmpeg.sox_effects.effect_names()
...
def apply_effects_tensor(
    ...
    Example - Torchscript-able transform
        ...
        >>>
        >>>> # Run transform
        >>> waveform, input_sample_rate = torchffmpeg.load("input.wav")
        >>> waveform, sample_rate = transform(waveform, input_sample_rate)
        >>> assert sample_rate == 8000
    """
    return torch.ops.torchffmpeg.sox_effects_apply_effects_tensor(tensor, sample_rate, effects, channels_first)
...
def apply_effects_file(
    ...
    Example - Apply random speed perturbation to dataset
        ...
        ...         ]
        ...         waveform, _ = torchffmpeg.sox_effects.apply_effects_file(
    ...
    if not torch.jit.is_scripting():
        if hasattr(path, "read"):
            ...
            ret = torchffmpeg.lib._torchffmpeg_sox.apply_effects_fileobj(path, effects, normalize, channels_first, format)
            if ret is None:
                raise RuntimeError("Failed to load audio from {}".format(path))
            return ret
        path = os.fspath(path)
    ret = torch.ops.torchffmpeg.sox_effects_apply_effects_file(path, effects, normalize, channels_first, format)
    if ret is not None:
        return ret
    raise RuntimeError("Failed to load audio from {}".format(path))
