...
from torchffmpeg.datasets.utils import _extract_zip
...
class DR_VCTK(Dataset):
    ...
    def _load_dr_vctk_item(self, filename: str) -> Tuple[Tensor, int, Tensor, int, str, str, str, int]:
        ...
        waveform_clean, sample_rate_clean = torchffmpeg.load(file_clean_audio)
        waveform_noisy, sample_rate_noisy = torchffmpeg.load(file_noisy_audio)
        return (
            waveform_clean,
            sample_rate_clean,
            waveform_noisy,
