...
def _get_local_path(key):
    path = Path(torch.hub.get_dir()) / "torchffmpeg" / Path(key)
    path.parent.mkdir(parents=True, exist_ok=True)
    return path


def _download(key, path, progress):
    url = f"https://download.pytorch.org/torchffmpeg/{key}"
...
def download_asset(
    ...
    *,
    ...
    """Download and store torchffmpeg assets to local file system.
    ...
    Note:
        Currently the valid key values are the route on ``download.pytorch.org/torchffmpeg``,
