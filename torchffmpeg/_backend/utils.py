...
import torch
import torchffmpeg.backend.soundfile_backend as soundfile_backend
from torchffmpeg._extension import _FFMPEG_INITIALIZED, _SOX_INITIALIZED
from torchffmpeg.backend.common import AudioMetaData

if _FFMPEG_INITIALIZED:
    from torchffmpeg.io._compat import info_audio, info_audio_fileobj, load_audio, load_audio_fileobj, save_audio
...
class SoXBackend(Backend):
    ...
    def info(uri: Union[BinaryIO, str, os.PathLike], format: Optional[str], buffer_size: int = 4096) -> AudioMetaData:
        if hasattr(uri, "read"):
            ...
            sinfo = torch.ops.torchffmpeg.sox_io_get_info(uri, format)
            if sinfo:
